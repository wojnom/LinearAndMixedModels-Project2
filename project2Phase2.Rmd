...
```
We can check now, whether adding interactions to the model will significantly improve our model or not.
```{r, warning = FALSE, message = FALSE}
anova(PhotoadditiveMouseTreat_noREML, crossedMouseTreatment)
```
Adding additional 95 degrees of freedom increases the LogLikelihood only by 1, so both AIC and BIC give same result as p-value.  We should get rid of interactions.

##Model diagnostics
Which fixed effects are important?
```{r, warning = FALSE, message = FALSE}
bestModel <- PhotoadditiveMouseTreat_noREML
...
qqmath(u)$`Animal:Study` #nie wiem czemu, nie dodaj¹ siê przedzia³y ufnoœci...
qqmath(u)[[2]]
